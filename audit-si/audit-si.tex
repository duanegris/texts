...
et sur des intitulés de matière qui peuvent avoir changé dans \motcle{APOGEE}.
...
de l'inscription de l'étudiant, comme par exemple le nombre d'inscrits, les 
...
par exmeple le nombre de diplômés en fin d'année, ou le nombre d'inscrits
l'an prochain.\\ 

SC-2 consiste à receuillir les notes après des enseignants et les saisir
...
de ces formations soient restreints (~25 étudiants par formation) est 
...
d'\motcle{Excel} pour gérer les données étudiants satisfait les beoins.
...
avec le RIB par la scolarité Master Universitaires, tandisi que
que la scolarité transmet également cette fiche mission au \srh
qui se charge de ventiler.\\
...
de toute la maquette dans \motcle{Apogée} permet de vérifier
rigoureusement la validité du processus de délivrance du diplôme.
...
correspond à un besoin du \srh (voir section \label{sc:rh-analyse}),
...
Il ressort de l'audit des services plusieurs constatations~:
...
L'entretie  avec \CK) a aussi mis en lumière
certains 
...
d'apporter des rpéonses aux demandes émanant des personnels.\\

\item \textbf{Travailler mode projet} : il faut convertir toutes les initiatives en projets,
c'est-à-dire définir les dates de début, de fin, les étapes de validation, eet les ressources
