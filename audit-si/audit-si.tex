...
et  enseignants.  C'est  le  projet \motcle{Alisée}.  L'intégration  du  système
...
des    différentes   instances    d'évaluations   (\motcle{AERES},    organismes
...
cas, l'interface web avec \motcle{SIFAC} ainsi que les contacts nécessaires avec
...
\CK   de    développer   une   fonctionnalité   de    l'\motcle{intranet}   pour
...
(Cécile Henner) du PGE IECS. L'outil est une base \motcle{Access} interfacée. La
...
cet audit  une mission  pour mettre  en place  un véritable  \motcle{CRM}. Cette
...
Le périmètre du \motcle{CRM} a été  volontairement limité au \sre. L'analyse des
besoins a montré que le \motcle{CRM} ne pouvait pas prendre en charge toutes les
...
exportée   (fréquence  mensuelle   environ)   vers  le   \motcle{CRM}  afin   de
l'enrichir.\\
...
système,  mais s'avère  limitante aujourd'hui.   Le nouveau  projet \motcle{CRM}
...
attendant  d'analyser le  produit \motcle{CRM}  qui sera  adopté dans  le schéma
...
récupérées directement d'ARIA mais la précision est plus faible et \motcle{Aria}
...
\item Dans la catégorie SC-2, NB développe de manière réactive des sites couplés
  ...
  \motcle{PHP}, \motcle{MySQL})  qui répondent aux besoins  ponctuels de gestion
  ...
  d'autres  sources  d'informations  (\motcle{intranet} EM  et  applications  du
  S.I. UdS).
...
informations administratives  contenues dans \motcle{Harpege}  et \motcle{SOSIE}
...
enregistrées dans un fichier \motcle{Excel} maintenu par EK.
...
le  S.I. de  l'UdS: application  \motcle{SOSIE}  pour les  heures effectuées  et
...
extractions  d'\motcle{HARPGest}  ne  peuvent  fournir que  les  noms,  date  de
naissance et nationalité d'une personne.  \motcle{SOSIE} ne liste que les heures
de cours pouvant donner  lieu à rétribution et sur des  intitulés de matière qui
peuvent avoir changé dans \motcle{Apogée}.   Pour connaître l'état des paiements
...
assez  efficacement aux  questions.  Cependant, le  fichier \motcle{Excel}  dans
...
(\motcle{SOSIE}  et \motcle{HARPGest}).  L'\motcle{ADE} permet  de vérifier  les
heures de cours effectuées. L'\motcle{intranet} EM n'est pas du tout utilisé par
le service RH.
...
inscrit.    Cette  procédure   utilise  exclusivement   l'\motcle{intranet}  EM.
...
Pour OP-3  seul un support  sous forme  d'un fichier \motcle{Excel}  non partagé
...
exploitables sont celles provenant de l'\motcle{intranet} ou que l'EM contrôle :
...
qui seront offertes par le futur  S.I. \motcle{Alisée} de l'UdS. Etant donné que
...
CC-3 consiste  en la saisie des  notes des jurys dans  un fichier \motcle{Excel}
...
la moyenne des candidats sous la  forme d'un fichier \motcle{Excel}. L'EM décide
...
un  fichier \motcle{Excel}  des noms  et adresses  de prospects  étudiants ayant
...
essentiellement  des fichiers  \motcle{Excel}.  Cette procédure  est imposée  et
...
\motcle{Access} qui  permet ensuite à \CK  de lancer des requêtes  permettant de
...
ouvertes sont  saisis par Aida  Saïd dans  des fichiers \motcle{Excel}.  Pour la
prospection   à   l'étranger,   l'EM   ne  dispose   que   de   la   plate-forme
\motcle{Pass-world}  de Passerelle  qui  permet de  recueillir des  candidatures
communes aux 8 écoles françaises participantes. \\
\end{itemize}


PP-2) L'outil  central pour cette  construction est l'\motcle{intranet}  avec la
...
version papier  entreposée dans  les scolarités. L'\motcle{intranet}  EM possède
...
pré-inscription par l'étudiant (application \motcle{Aria}).\\
...
De même, l'\motcle{intranet}, avant de  connaître les capacités du futur système
\motcle{Alisée},  semble  être   l'outil  le  plus  adéquat   pour  ajouter  des
...
\motcle{Apogée}  lors de  l'inscription de  l'étudiant. Ce  sont par  exemple le
...
Pour SC-1, la plupart des  informations demandées sont dans \motcle{Apogée}.  La
...
des fichiers  \motcle{Excel} en parallèle  afin de répondre plus  facilement aux
...
charge  une formation  complète. L'utilisation  d'\motcle{Excel} pour  gérer les
données  étudiants satisfait  les besoins.   Il faut  noter que  l'extraction de
fichiers au  format \motcle{Excel} est  possible, mais ne semble  pas facilement
