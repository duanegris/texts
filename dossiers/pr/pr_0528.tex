...
entreprises, le  \emph{startup week-end}),  ou à  l'extérieur (la  \emph{nuit de
  l'info}, le forum AlsaceTech). Depuis  septembre 2012, Pierre Tellier m'a aidé
pour    conduire    la    transition     dans    nos    relations    avec    les
...
partie  des  entreprises  cette  année écoulée.  Ces  rencontres  permettent  de
réaffirmer la  volonté de  l'école d'être  bien ancré  dans le  tissu industriel
local pour fournir des cadres de haut niveau aux entreprises.
...
formations universitaires et  des laboratoires de recherche fera  de l'école une
brique   importante   de   l'ensemble.   L'existence   de   l'antenne   renforce
l'attractivité    du     site    Strasbourgeois     dans    le     domaine    de
l'informatique. L'ENSIIE  attire en  effet des  étudiants au  niveau local  : la
formation  représente  pour  les  meilleurs étudiants  du  DUT  informatique  de
l'Université de  Strasbourg, une  poursuite d'études  attractive à  laquelle ils
souscrivent largement alors qu'ils  partaient auparavant pour rejoindre d'autres
écoles  d'ingénieurs en  France. L'ENSIIE  attire également  au niveau  national
(61\%  hors  Alsace à  cette  rentrée),  voire  international (Campus  France  +
conventions internationales comme Xidian,  issues du partenariat Mines-Télécom).
C'est  actuellement la  seule formation  en  informatique à  Strasbourg avec  un
rayonnement  aussi  large.   L'ENSIIE  attire   des  étudiants  à  proximité  de
l'Université,   et   l'antenne   doit   s'appuyer  sur   les   compétences   des
ensiegnants-chercheurs  de   l'Université  de   Strasbourg.   Ceci   permet  une
mutualisation efficace  des enseignements lors des  semestres 4 et 5  composés à
80\%  d'options,  et de  manière  plus  générale,  favorise la  proximité  entre
enseignants-chercheurs de l'Université et élèves  de l'ENSIIE qui est nécessaire
pour attirer ces élèves vers la recherche.  C'est pour cela que nous travaillons
actuellement  au rétablissement  de la  convention qui  avait permis  aux EC  de
l'université  d'inclure   les  heures   d'enseignement  à  l'ENSIIE   dans  leur
service. En  contre-partie, L'ENSIIE peut  également être un soutien  à d'autres
formations, comme  dans le projet  de création d'un nouveau  diplôme d'ingénieur
\textit{Internet des objets} porté par  Télécom Physique Strasbourg, pour lequel
l'ENSIIE assurerait la première  année d'enseignement.  Concernant la recherhce,
la présence des EC de l'Université  dans la formation amène davantage d'élèves à
faire des  stages recherche  dans les  équipes de  Icube ou  de l'IRMA  avec des
poursuites éventuelles en  thèse.  Deux étudiants (dont un  provenant d'Evry ont
déjà entamé une  thèse à l'Université de Strasbourg). La  bonne connaissance des
thèmes  de recherche  à  l'Université  est donc  indispensable  au directeur  de
l'ENSIIE Strasbourg  pour insérer  correctement l'école dans  cet environnement.
Le fléchage de ce poste en  informatique pour le laboratoire Icube va exactement
dans ce sens.
...
scientifiques,  essentiellement des  programmes MPI~\footnote{%  Message Passing
  Interface définit  une bibliothèque  de fonctions  permettant à  des processus
  d'échanger des messages.  C'est devenu  le standard \textit{de facto} pour les
  programmes parallèles  exécutés sur  des architectures à  mémoire distribuée.}
sur  les  grilles.  Deux  catégories  de  problèmes  ont  été traitées  dans  ce
projet. D'une part, déterminer quelles \emph{performances} on peut espérer d'une
application exécutée sur des ressources  hétérogènes distribuées à large échelle
géographique et  ce qu'on peut faire  pour les améliorer.  D'autre  part, quelle
couche \emph{intergicielle} peut  on proposer pour masquer la  complexité de ces
systèmes répartis et améliorer la prise en charge des applications.


\paragraph{Performances} 

Pour améliorer les performances, nosu  avons étudié les techniques d'équilibrage
de charge statiques  ou dynamiques.  En effet, les découpages  des données et la
distribution  de  la charge  dans  de  nombreuses applications  parallèles  font
l'hypothèse que les processeurs et les réseaux sont homogènes. Pour optimiser le
temps  de  calcul  d'un  ensemble  de tâches  indépendantes  exécutées  sur  des
processeurs  hétérogène, il  faut déterminer  la meilleure  quantité de  données
distribuées  à  chacune. Nous  avons  proposé  des algorithmes  pour  déterminer
statiquement de telles  distributions optimales. Ces algorithmes  ont été testés
sur  de   vraies  applications,   dont  l'application  de   géophysique  décrite
précédemment~\cite{icps-2002-62,icps-2003-75,icps-2004-125}.      Nous     avons
également comparé cette  approche statique à l'approche  dynamique dans laquelle
le maître distribue un nouveau bloc de  travail à un esclave dès que celui-ci le
demande : l'équilibrage de charge se fait alors naturellement selon le rythme de
calcul des  esclaves. La  difficulté dans  cette approche  est de  déterminer la
taille  du  bloc  à envoyer:  en  dessous  de  la  taille  idéale, il  y  a  des
aller-retours  inutiles, tandis  qu'au  dessus, les  esclaves  ne finissent  pas
...
\textit{Mon projet pour la recherche  s'inscrit naturellement dans la continuité
  ...
  Image et  Calcul Parallèle  Scientifique (ICPS).   Je souhaite  développer mes
  activités de  recherche dans ce  laboratoire, tout  en les inscrivant  dans le
  cadre du LabEx IRMIA auquel l'équipe participe.}\\
...
\subsubsection{Thème Applications}
...
employé au  CEA Cadarache. Nous  co-encadrons depuis février 2011  un doctorant,
Matthieu Kuhn,  avec Nicolas  Crouseille chargé  de recherches  en mathématiques
appliquées dans  l'équipe CalVi\footnote{L'équipe-projet CalVi  est bi-localisée
  entre  Strasbourg  et  Nancy.   Nos  collaborateurs  directs  font  partie  du
  laboratoire de mathématiques IRMA (UMR  7501 CNRS-UdS).}, pour paralléliser un
code de physique  de plasmas de bords. Ce travail  renforce une collaboration de
longue date  (depuis 2002) entre  l'ICPS et CalVi.  Cette collaboration a  été à
l'origine de notre association à la proposition de LabEx avec l'IRMA. Ce LabEx a
été  obtenu en  février  2012, et  va  donc permettre  de  renforcer de  manière
significative ces  travaux interdisciplinaires,  notamment à travers  des thèses
co-encadrées. Nous souhaitons  également recruter un permanent sur  ce thème. Un
poste  de CR2  CNRS ayant  été  fléché pour  recruter un  informaticien dans  un
laboratoire de mathématiques,  nous avons travaillé en  concertation avec l'IRMA
pour porter un dossier de candidature calibré pour ces besoins à ce concours.\\
...
calculs  très nombreux  mais  indépendants,  et utilisant  de  grandes bases  de
...
capacités de calcul qu'offre  l'organisation virtuelle BioMed (sous-ensemble des
ressources de la grille européenne EGI). Cette collaboration est importante pour
les   protéomistes  Strasbourgeois   qui   ont  obtenu   la  responsabilité   de
...
``Infrastructures nationales de recherche en biologie''. Trois étudiants sont en
stage  depuis février  2012 pour  apporter  des améliorations  à la  plate-forme
développée à l'IPHC~\cite{iphc-2011} ou faire  un état de l'art des technologies
de cloud et tester leur introduction dans cet environnement de production.
 



\subsubsection{Thème Grilles et Clouds}
...
\subsubsection{Complémentarité des thèmes}
...
\includepdf[pages=1-2]{fiche_candidature_mc4013.pdf}
