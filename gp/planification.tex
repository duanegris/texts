...
\frame{
\frametitle{PERT Probabiliste}

\begin{itemize}
\item<+-> Objectif: Inclure risque et incertitude dans la durée

\item<+-> Durée d'une tâche considérée comme une variable aléatoire.
Des études ont montré que la durée d'une tâche peut être modélisée une loi Beta.

\item<+-> 
La durée d'un chemin est la somme de telles variables aléatoires.\\
Théorème centrale limite $\Rightarrow$
La durée d'un chemin suit une loi normale.
%--------- comment -------------
...
\frame{
\frametitle{PERT Probabiliste en pratique}

Les travaux de C. Clarke (1962) ont donné une méthode pour contrôler
les paramètres de la loi de distribution Beta $\alpha$ et $\beta$ 
à partir de 3 paramètres plus simples : \\
\pgfputat{\pgfxy(7,-3)}{\pgfbox[left,base]{\includegraphics[width=4cm]{fig/betapdf_general.png}}}
\pause
...
\frametitle{PERT probabiliste (4)}
	...
	\textbf{Idée}: on cherche une borne supérieure $t$ de la durée d'un chemin
       avec un certain degré de confiance $p$.\\

\begin{center}
\includegraphics[width=8cm]{fig/gauss_pdf_cdf.png}
\end{center}

	Soit $F(t)$ est la fonction de répartition (ou CFD) de ${\cal N}(0,1)$,\\ 
      on cherche
	\begin{center}
	$t$ telle que $F(t) \leq p$
\end{center}
}

\frame{
\frametitle{PERT probabiliste (5)}

Le comportement stochastique s'applique à \\
l'incertitude déclarée sur le chemnin : $E$.

\vspace{.5cm}

Si on appelle $G = F^{-1}$,\
\begin{block}{}
La durée maximum du chemin avec une probabilité $p$ est:
	\[ {\cal D}(p) = D + E \times G(p) \] 
\end{block}

On utilise une table (ou calculatrice):
\begin{small}
