...
%% --- etape 4
\onslide<4->{
	\node (chefprojet) at (0,-2) {\includegraphics[height=9mm] {fig/user_female.png}};
	\draw (0,-2.5) node[below]{{\small Chef de projet}};
	\draw[thick,->] (chefprojet) -- (projet);
}
\onslide<5->{
	\node (decideur) at (0,2) {\includegraphics[height=9mm] {fig/user_executive.png}};
	\draw[thick,->] (decideur) -- (projet)
			    node[near start,above]{\small Décideur};
}
\onslide<6->{
	\node (equipe) at (2,-2) {\includegraphics[height=9mm] {fig/user_group.png}};
